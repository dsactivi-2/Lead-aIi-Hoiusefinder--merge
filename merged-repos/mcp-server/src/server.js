...
async function sshExec(command) {
    try {
        const { stdout, stderr } = await execAsync(`ssh -i ${SSH_KEY} -o StrictHostKeyChecking=accept-new ${SSH_HOST} "${command.replace(/"/g, '\\"')}"`, { timeout: 30000 });
...
// Tool: health_check
server.tool("health_check", { baseUrl: z.string().optional().describe("Override backend base url") }, async ({ baseUrl }) => {
...
server.tool("server_stats", {}, async () => {
...
// Tool: db_query - Execute read-only SQL query
server.tool("db_query", {
    query: z.string().min(1).describe("SQL query (SELECT only)"),
    database: z.string().optional().describe("Database name (default: cloud_agents)"),
    ...
    const db = database ?? "cloud_agents";
    const escapedQuery = query.replace(/'/g, "'\\''");
    const cmd = `psql -d ${db} -c '${escapedQuery}'`;
    const out = await sshExec(cmd);
    return {
...
server.tool("audit_events_stats", {
